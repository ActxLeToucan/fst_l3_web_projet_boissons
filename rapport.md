...
L'utilisation de VueJS a été envisagée, mais nous avons jugé le gain de temps que cela nous procurerait trop faible pour justifier son installation.
...
Lors du remplissage, les tables `recipe`, `ingredient`, `aliment` et `category` sont remplies à partir du fichier [src/assets/Donnees.inc.php](./src/assets/Donnees.inc.php).
...
<img style="display: block; margin: 0 auto" alt="Elements graphiques" src="./conception/UI.png"/>

## Recherche
Lors de la recherche d'un cocktail, l'utilisateur peut rechercher un nom, mais aussi filtrer par ingrédient.
### Front
La recherche d'aliments dans le site web est représentée par deux éléments principaux :
...
La représentation des aliments nous a pris un certain temps à mettre en place, car il a fallu développer un système de menu/sous-menu pour afficher les aliments de manière hiérarchique, ainsi que pouvoir en sélectionner plusieurs et dire s'ils étaient voulus ou non dans les cocktails.
...
Cela nous permet donc d'afficher les erreurs directement dans le formulaire, sans avoir à recharger la page ou de mettre des paramètres supplémentaires dans les requêtes (comme le token utilisateur dans les headers).
...
- Favoris locaux : Les favoris sont stockés dans le navigateur de l'utilisateur. Ils sont donc accessibles uniquement depuis le navigateur utilisé pour les ajouter.
...
Un problème s'est posé lors de la mise en place des favoris serveur : Comment synchroniser les favoris locaux de l'utilisateur au moment de la création d'un compte ?
Pour cela, nous avons opté pour une fenêtre modale qui s'affiche au moment de la création du compte, et qui demande à l'utilisateur s'il souhaite synchroniser ses favoris locaux avec son nouveau compte.
...
## Connexion
### Front
La création et la connexion à un compte sont gérées par l'API, le site web ne fais office que d'interface pour récupérer les champs saisis et envoyer les données.
...
Les formulaires personnalisés sont utilisés pour la connexion et l'inscription, une requête est donc faite avec la classe API.js pour ces deux fonctionnalités.
...
Comme demandé, être connecté est optionnel pour utiliser le site.

Le seul avantage de se connecter est de pouvoir sauvegarder ses cocktails favoris sur le serveur et de pouvoir les retrouver sur n'importe quel navigateur.

## Recherche
La recherche est effectuée lorsque vous tapez pour vous fournir des propositions de cocktail et vous fournir les résultats.
Vous pouvez rechercher par nom de cocktail. Vous avez aussi la possibilité de filtrer par ingrédient en sélectionnant un aliment de l'arborescence (dans la partie "Aliments :", sous la barre de recherche).

L'aliment sélectionné est ajouté. Les cocktails **doivent** contenir cet ingrédient pour être affichés. Si vous souhaitez interdire cet aliment, vous pouvez cliquer sur le bouton "+" de l'aliment pour le passer en "-".

L'ingrédient est barré. Les cocktails **ne doivent pas** contenir cet ingrédient pour être affichés.
Vous pouvez ajouter autant d'ingrédients que vous le souhaitez, et les retirer en cliquant sur le bouton "x" de l'ingrédient.
Vous ne pouvez sélectionner un ingrédient qui a un parent sélectionné.

Un cocktail vous correspond à ce que vous cherchez ? Cliquez dans les propositions de recherches en dessous de la barre de recherche, ou sélectionnez la carte du cocktail dans la liste des résultats pour afficher les détails du cocktail.

## Détails du cocktail
Depuis les détails du cocktail, vous pouvez l'ajouter à vos favoris locaux (si vous n'êtes pas connecté) ou sur le serveur (si vous êtes connecté).

Vous appréciez un des ingrédients du cocktail ? Cliquez sur l'ingrédient pour afficher les cocktails qui contiennent cet ingrédient !